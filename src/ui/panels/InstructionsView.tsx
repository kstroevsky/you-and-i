...
export function InstructionsView({ language }: InstructionsViewProps) {
  ...
  return (
    <section className="instructions-panel">
      <div className="instructions-panel__header">
      ...
      <div className="instructions-content">
        {blocks.map((block, index) => {
          if (block.type === 'divider') {
            return <hr key={`divider-${index}`} className="instructions-divider" />;
          }

          if (block.type === 'heading') {
            const HeadingTag = block.level === 1 ? 'h2' : block.level === 2 ? 'h3' : 'h4';
            return (
              <HeadingTag key={`heading-${index}`} className={`instructions-heading instructions-heading--${block.level}`}>
          ...
          if (block.type === 'paragraph') {
            return (
              <p key={`paragraph-${index}`} className="instructions-paragraph">
                {renderInline(block.text)}
              </p>
            );
          }
          ...
          return (
            <ListTag key={`list-${index}`} className="instructions-list">
